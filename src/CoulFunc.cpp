...
CoulFunc::CoulFunc(int z1,int z2, double redmass, bool useGSLFunctions) : 
  z1_(z1), z2_(z2), redmass_(redmass), useGSLFunctions_(useGSLFunctions) {
  lLast_=0;
  radiusLast_=0.0;
  energyLast_=0.0;
  coulLast_.F=0.0;
...
void CoulFunc::GSLErrorHandler(const char* reason, 
			       const char* file,
			       int line,
			       int errorCode) {
  if(errorCode==GSL_EOVRFLW) return;
  std::cout << "GSL Error: " << reason << '.' << std::endl;
  std::cout << "On line " << line   << " of " << file << '.' << std::endl;
  std::cout << "Continuing running, check output." << std::endl;
