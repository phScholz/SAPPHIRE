...
int main(int argc, char *argv[]) {
  ...
  parseCommandLineForOptions(args,suffixNo,preEq,
                             numPiParticles,numPiHoles,numNuParticles,numNuHoles,
			     calcAverageWidth,calcRates,asciiIn,inFile,entranceState,exitStates,
			     printTrans);
  ...
  if(args.size()<4) {
    ...
    if(!asciiIn) {
      ...
      if(pType==0) 
	std::cout << std::setw(14) << "projectile:"      
		  << std::setw(12) << "g"      << std::setw(0) << std::endl;
      else if(pType==1) 
	std::cout << std::setw(14) << "projectile:"      
		  << std::setw(12) << "n"      << std::setw(0) << std::endl;
      else if(pType==2) 
	std::cout << std::setw(14) << "projectile:"      
		  << std::setw(12) << "p"      << std::setw(0) << std::endl;
      else if(pType==3) 
	std::cout << std::setw(14) << "projectile:"      
		  << std::setw(12) << "a"      << std::setw(0) << std::endl;
    ...
    for(int i = 0;i<loopMax;++i) {
      ...
      CrossSection* crossSection = (asciiIn) ?
	new CrossSection(entrancePairs[i].Z_,entrancePairs[i].A_,
			 entrancePairs[i].pType_,energyFile,calcRates) :
	new CrossSection(Z,A,pType,energyFile,calcRates,entranceState,exitStates);
